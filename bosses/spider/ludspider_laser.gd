...
func _physics_process(delta):
	...
	$CollisionCPUParticles2D.emitting = _is_colliding_with_player(delta)
	
	if _is_colliding_with_player(delta):
...
func _is_colliding_with_player(delta):
	if is_colliding():
		var collider = get_collider()
		
		if collider.is_in_group("destroy_enemy_bullet") && !(collider.is_in_group("is_invincible") || collider.is_in_group("is_iframes")):
